...
describe("TicketFactory", function() {
  ...
  before(async function() {
    ...
    factory = await ethers.getContractFactory("TicketFactory")
    TicketFactory = await factory.deploy()


    const tx = await TicketFactory.createTicketGroup(
    ...
    const event = rc.events.find(event => event.event === "TicketGroupCreated")
    const [id, creator] = event.args
    const TicketGroupAddress = await TicketFactory.getTicketGroup(id)
    

    TicketGroup = await ethers.getContractAt("TicketGroup", TicketGroupAddress)
